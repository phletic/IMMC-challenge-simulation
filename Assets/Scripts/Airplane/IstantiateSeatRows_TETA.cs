...
public class IstantiateSeatRows_TETA : MonoBehaviour
{

    public int typePlane;
    public PlaneDimensionController planeDimensionController;
    public IsSeat[] typeArrangements = { new NarrowBodyPlaneSeats(), new FlyingWingPlaneSeat(), new TwoATwoSeat() };

    public IsSeat _isSeat;
    public GameObject seatPrefab;
    public static List<GameObject> SeatArrangement;

    public SpawnPerson_TETA _spawnPerson;
    ...
    void Start()
    {
        ...
        for (int x = 0; x < planeDimensionController.width; x++)
        {
            for (int y = 0; y < planeDimensionController.length; y++)
            {
                ...
                {
                    GameObject clone = Instantiate(seatPrefab, new Vector3(y, 10f, x), Quaternion.identity);
                    Storage.numberSeats++;
